...
(function()
{
    ...
    {
        $('.dtable a[href*="story-view-"], .dtable a[href*="requirement-view-"], .dtable a[href*="m=story"][href*="f=view"], .dtable a[href*="m=requirement"][href*="f=view"], .dtable a[href*="execution-storyview-"]').each(function()
        {
            var href = $(this).attr('href') || '';
            var match = href.match(/(?:story|requirement)-view-(\\d+)/) || href.match(/execution-storyview-(\\d+)/) || href.match(/[?&]storyID=(\\d+)/);
            if(!match) return;
            var id = match[1];
    ...
    if(window.waitDom) window.waitDom('.dtable a[href*="story-view-"], .dtable a[href*="requirement-view-"], .dtable a[href*="m=story"][href*="f=view"], .dtable a[href*="m=requirement"][href*="f=view"], .dtable a[href*="execution-storyview-"]', observeObjectEffort);
})();
</script>
EOT;
}
