...
ensure_bi_builtin_data()
{
    ...
    local has_bi_tables
    has_bi_tables="$("${app_mysql[@]}" -Nse "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='${DB_NAME}' AND table_name IN ('${DB_PREFIX}dimension', '${DB_PREFIX}screen')" 2>/dev/null || true)"
    [[ "$has_bi_tables" == 2 ]] || return 0

    echo "→ 补齐 BI 初始化数据"
    "${app_mysql[@]}" <<SQL
REPLACE INTO \`${DB_PREFIX}dimension\` (id, name, code, \`desc\`, createdBy, createdDate, editedBy, editedDate, deleted) VALUES
...
SQL
    ...
    PROJECT_ROOT="$PROJECT_ROOT" DB_PREFIX="$DB_PREFIX" php <<'PHP' | "${app_mysql[@]}"
<?php
