...
class taskModel extends model
{
    ...
    public function create(object $task, bool $createAction = true): false|int
    {
        ...
        /* An auto-increment ID can be reused after a restore or table rebuild while an orphan task spec remains. */
        $this->dao->delete()->from(TABLE_TASKSPEC)->where('task')->eq($taskID)->exec();
        if(dao::isError()) return false;
        $this->dao->insert(TABLE_TASKSPEC)->data($taskSpec)->autoCheck()->exec();
