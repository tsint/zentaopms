...
(function()
{
    ...
    {
        $('.dtable a[href*="story-view-"], .dtable a[href*="requirement-view-"], .dtable a[href*="m=story"][href*="f=view"], .dtable a[href*="m=requirement"][href*="f=view"], .dtable a[href*="projectstory-view-"]').each(function()
        {
            var href = $(this).attr('href') || '';
            var match = href.match(/(?:story|requirement)-view-(\\d+)/) || href.match(/projectstory-view-(\\d+)/) || href.match(/[?&]storyID=(\\d+)/);
            if(!match) return;
            var id = match[1];
    ...
    {
        injectObjectEffort();
        var target = document.getElementById('projectStories') || document.getElementById('stories') || document.querySelector('.dtable');
    ...
    if(window.waitDom) window.waitDom('.dtable a[href*="story-view-"], .dtable a[href*="requirement-view-"], .dtable a[href*="m=story"][href*="f=view"], .dtable a[href*="m=requirement"][href*="f=view"], .dtable a[href*="projectstory-view-"]', observeObjectEffort);
})();
</script>
EOT;
}
