...
- 非法耗时被拒绝 consumed 字段报错 @耗时必须为大于 0 的数字。
- 未来日期被拒绝 @日期不能为空，且不能晚于今天。
- 关闭对象被拒绝 @已关闭对象不能登记工时。
- Story 登记工时后返回 ID 大于 0 @1
- 多执行需求未指定执行时被拒绝 @请选择需求所属的执行。
- 需求不能登记到未关联执行 @选择的执行未关联当前需求。
- 需求工时只计入选定执行 @4,3,1
- 未排期需求工时归入项目而不伪造执行 @900001,0
- 执行汇总 estimate/consumed/left @10,5,4
- 项目汇总包含未排期需求 estimate/consumed/left @14,8,5
- 项目统计被对象工时刷新 @14.00,8.00,5.00,61.50
- 执行统计被对象工时刷新 @10.00,5.00,4.00,55.50
- 燃尽图计算包含对象工时 @10,4,5
- 新增需求后燃尽范围故事点上升 @13
- 编辑后 consumed/left 变化 @4,1
- 删除后记录软删除 @1
- 核心任务表未被对象工时修改 @0.00
- 无登记权限的用户被拒绝 @1
- 有登记权限的用户可登记 @1
...
- 所有者有编辑权限时可操作 @1
- 非所有者即使有编辑权限也被拒绝 @1
- 关闭对象后燃尽汇总清零预计和剩余但保留耗时 @0,6,0
- 关闭动作立即刷新项目剩余工时 @1.00
- 关闭动作立即刷新当日燃尽 @0.00,6.00,0.00
