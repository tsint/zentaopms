...
$syncURL    = $this->createLink('statetransition', 'syncGlobal', "objectType={$objectType}&productID={$productID}");
...
$syncBtn  = $productID > 0 ? '<button type="button" class="btn btn-lg statetransition-sync-global-btn" data-sync-url="' . htmlspecialchars($syncURL) . '" data-return-url="' . htmlspecialchars($this->createLink('statetransition', 'browse', "objectType={$objectType}&productID={$productID}")) . '"><i class="icon icon-copy"></i> ' . htmlspecialchars($lang->statetransition->syncFromGlobal) . '</button>' : '';
...
$confirmSyncJSON  = json_encode($lang->statetransition->confirmSyncFromGlobal, JSON_UNESCAPED_UNICODE);
$mermaidWebPathJSON = json_encode($mermaidWebPath, JSON_UNESCAPED_SLASHES);
html('<script>(function(){var seq=(window.__statetransitionBrowseMermaidSeq||0)+1;window.__statetransitionBrowseMermaidSeq=seq;function postAction(url,successUrl,fallback){var body=new FormData();body.append("confirm","1");fetch(url,{method:"POST",body:body,headers:{"X-Requested-With":"XMLHttpRequest"}}).then(function(resp){return resp.json();}).then(function(result){if(result.result==="success"){window.location.href=successUrl;}else{alert(result.message||fallback);}}).catch(function(e){alert("网络错误: "+e.message);});}function bindActions(){document.querySelectorAll(".statetransition-reset-btn").forEach(function(btn){if(btn.dataset.bound)return;btn.dataset.bound="1";btn.addEventListener("click",function(){if(!confirm(' . $confirmResetJSON . '))return;postAction(btn.dataset.resetUrl,btn.dataset.returnUrl,"恢复默认失败");});});document.querySelectorAll(".statetransition-sync-global-btn").forEach(function(btn){if(btn.dataset.bound)return;btn.dataset.bound="1";btn.addEventListener("click",function(){if(!confirm(' . $confirmSyncJSON . '))return;postAction(btn.dataset.syncUrl,btn.dataset.returnUrl,"同步全局失败");});});}function loadMermaid(done){if(typeof mermaid!=="undefined")return done();var id="statetransitionMermaidLib";var script=document.getElementById(id);if(script){script.addEventListener("load",done,{once:true});return;}script=document.createElement("script");script.id=id;script.src=' . $mermaidWebPathJSON . ';script.onload=done;document.head.appendChild(script);}function renderMermaid(){if(seq!==window.__statetransitionBrowseMermaidSeq)return;var nodes=Array.from(document.querySelectorAll(".statetransition-flow .statetransition-mermaid"));if(!nodes.length)return;if(typeof mermaid==="undefined"){setTimeout(renderMermaid,50);return;}try{mermaid.initialize({startOnLoad:false,securityLevel:"loose"});nodes.forEach(function(node){node.classList.remove("mermaid");node.textContent=node.dataset.mermaidSource||node.textContent;node.removeAttribute("data-processed");});var promise=mermaid.run?mermaid.run({nodes:nodes}):new Promise(function(resolve){mermaid.init(undefined,nodes);resolve();});promise.catch(function(e){if(seq!==window.__statetransitionBrowseMermaidSeq)return;console.warn("[statetransition] mermaid render error:",e);nodes.forEach(function(node){node.innerHTML="<div class=\"workflow-hint\">流程图渲染失败，请检查定义。</div>";});});}catch(e){console.warn("[statetransition] mermaid render error:",e);}}function schedule(){bindActions();loadMermaid(function(){setTimeout(renderMermaid,0);});}if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",schedule,{once:true});}else{schedule();}})();</script>');
