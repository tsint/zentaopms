...
# MySQL 连接参数
mysql_root_opts="--protocol=TCP --ssl=0 -h $DB_HOST -P $DB_PORT -u $DB_ROOT_USER --default-character-set=utf8mb4"
...
for attempt in $(seq 1 30); do
    if mysql $mysql_root_opts -e 'SELECT 1' >/dev/null 2>&1; then break; fi
...
# 检查是否已初始化
admin_exists=$(mysql $mysql_root_opts -Nse "SELECT COUNT(*) FROM \`$DB_NAME\`.\`${DB_PREFIX}user\` WHERE account='$ADMIN_ACCOUNT'" 2>/dev/null || echo 0)
...
app_mysql_opts="--protocol=TCP --ssl=0 -h $DB_HOST -P $DB_PORT -u $DB_USER -p$DB_PASSWORD --default-character-set=utf8mb4 $DB_NAME"

# 导入核心 schema
if ! mysql $app_mysql_opts -e "SELECT 1 FROM \`${DB_PREFIX}user\` LIMIT 1" >/dev/null 2>&1; then
    echo "  导入核心 schema..."
    sed -e "s/\`zt_/\`${DB_PREFIX}/g" -e "s/\`ztv_/\`${DB_PREFIX}v_/g" "$PROJECT_ROOT/db/zentao.sql" | mysql $app_mysql_opts
fi

# 安装扩展
for extension in objecteffort workflowflowchart; do
    install_sql="$PROJECT_ROOT/extension/custom/$extension/db/install.sql"
    if [[ -f "$install_sql" ]] && ! mysql $app_mysql_opts -e "SELECT 1 FROM \`${DB_PREFIX}${extension}\` LIMIT 1" >/dev/null 2>&1; then
        echo "  安装 $extension..."
        sed -e "s/\`zt_/\`${DB_PREFIX}/g" "$install_sql" | mysql $app_mysql_opts
    fi
done

# 安装 statetransition
if [[ -f "$PROJECT_ROOT/module/statetransition/db/install.sql" ]] && ! mysql $app_mysql_opts -e "SELECT 1 FROM \`${DB_PREFIX}workflow_definition\` LIMIT 1" >/dev/null 2>&1; then
    echo "  安装 statetransition..."
    sed -e "s/\`zt_/\`${DB_PREFIX}/g" "$PROJECT_ROOT/module/statetransition/db/install.sql" | mysql $app_mysql_opts
fi

# 创建管理员
echo "→ 创建管理员账号"
password_hash=$(php -r "echo md5('$ADMIN_PASSWORD');")
mysql $app_mysql_opts <<SQL
